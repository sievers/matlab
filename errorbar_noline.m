function [hh,hhh] = errorbar_noline(varargin)
...
function [h,hhh] = Lerrorbarv6(varargin)
...
symbol = [ mark col]; % Use marker only on data part
%symbol = [ls mark col]; % Use marker only on data part

esymbol = [ls col]; % Make sure bars are solid
%esymbol = ['-' col]; % Make sure bars are solid

%disp('Hello');
h = plot('v6',xb,yb,esymbol,'parent',cax); hold(cax,'on')
h = [h;plot('v6',xc,yc,['-' col],'parent',cax)]; 
