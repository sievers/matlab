...
if (do_manual)
    ...
    if (sum((x>=fwee(1))&(x<=fwee(2)))==0) %no data points
        h=-1;
        return;
    end
    tee=(fwee(2)-fwee(1))/100;
    y=y((x>=fwee(1))&(x<=fwee(2)));
    l=l((x>=fwee(1))&(x<=fwee(2)));
    u=u((x>=fwee(1))&(x<=fwee(2)));
    x=x((x>=fwee(1))&(x<=fwee(2)));
    npt=length(x);
else
    tee = (max(x(:))-min(x(:)))/100;  % make tee .02 x-distance for error bars
end
