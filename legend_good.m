...
elseif nargout==4
    [leg,labelhandles,outH,outM] = find_legend_info(ha);
elseif nargout>0
    error(id('BadNumberOfOutputs'),'Must have 0,1,2 or 4 outputs.');
end
...
if isempty(children)
    auto_children = true;
    children = graph2dhelper ('get_legendable_children', ha, false);
    % if still no children, return empty
    if isempty(children)
        warnmsg = struct('identifier',id('PlotEmpty'),...
                         'message','Plot empty.');
...
%whos 
%keepvec
children=children(keepvec);
strings=strings(keepvec);
...
while n <= nargs
  if ischar(argin{n})
    if strcmpi(argin{n},'orientation') && (n < nargs) && ...
          ischar(argin{n+1}) && ...
    ...
    elseif strcmpi(argin{n},'location') && (n < nargs) && ...
  ...
  elseif isnumeric(argin{n}) && length(argin{n})==4 && ...
    ...
    % This is an undocumented API for backwards compatiblity with
    % Basic Fitting.
    position = argin{n};
...
%----------------------------------------------------------------%
function ax=get_data_axes(a)

if isprop(handle(a),'shapetype')
    if strcmpi(get(handle(a),'shapetype'),'legend') || ...
        strcmpi(get(handle(a),'shapetype'),'colorbar')
        error(id('InvalidDataPeerParameter'),...
            'Plot axes must not be a legend or colorbar.');
    end
    ax = get(handle(a),'axes');
else
    ax=a;
end
...
if mod(n,2)==0 % must be even
    ...
    for i=1:2:n
        if ~ischar(args{i})
            error(id('InvalidPropertyName'),...
                  'Property names must be strings.')
        elseif ~isprop('scribe','legend',args{i})
            error(id('UnknownProperty'),...
                  sprintf('Unknown property: %s',args{i}));
        end
    end
else
    error(id('EvenPropertyValuePairs'),...
          'Must supply an even number of property/value pairs.');
end
...
%----------------------------------------------------------------%
function found=gotscattergroup(list,group)

i=1; found=0;
while ~found && i<=length(list)
    if isequal(group,list(i))
        found=1;
    end
    i = i+1;
end
...
if isempty(str)
  ...
  if length(str) ~= length(ch)
    if ~auto_children || length(str) > length(ch)
      msg.message = 'Ignoring extra legend entries.';
      msg.identifier = id('IgnoringExtraEntries');
    end
...
% for j=1:length(ch),
%     if (length(str{j})==0)
%         disp(['cutting set ' num2str(j) str{j}]);
%         ch(j)=0;
%     else
%         disp(['keeping set ' num2str(j) str{j}]);        
%     end
% end
% ch=ch(ch~=0);

function str = id(str)
str = ['MATLAB:legend:' str];
